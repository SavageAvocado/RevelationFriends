...
public class FriendCmd extends AsyncCommand {
    ...
    protected void command(CommandSender sender, String... args) {
        ...
        if (args.length == 0) {
            MessageUtils.message(user, this.plugin().getConfig(RevelationFriends.ConfigType.LANG).getString("error.invalid-arguments").replace("%cmd%", "help"));
            return;
        }
        ...
        if (command == null) {
            MessageUtils.message(user, this.plugin().getConfig(RevelationFriends.ConfigType.LANG).getString("error.invalid-arguments").replace("%cmd%", this.getSuggestion(args[0])));
            return;
        }
