...
public class JoinCmd extends SubCommand {
    ...
    public void execute(CommandSender sender, String... args) {
        ...
        if (args.length == 1) {
            MessageUtils.message(user, this.plugin().getConfig(RevelationFriends.ConfigType.LANG).getString("error.invalid-arguments").replace("%cmd%", "join <player>"));
            return;
        }

        ProxiedPlayer friend = this.plugin().getProxy().getPlayer(args[1]);
        ...
        if (!this.plugin().getUserManager().get(user.getUniqueId()).isFriends(friend.getUniqueId())) {
